...
class OverlayManagerShellCommand extends ShellCommand {
    ...
    private int runList() throws RemoteException {
        ...
        Map<String, List<OverlayInfo>> stuff = mInterface.getAllOverlays(userId);
        for (String targetPackageName : stuff.keySet()) {
            out.println(targetPackageName);
            for (OverlayInfo oi : stuff.get(targetPackageName)) {
                String status = "---";
                if (oi.isApproved()) {
                    status = "[ ]";
                }
